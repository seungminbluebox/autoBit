...
공통 전략 엔진을 사용하는 실전 연결 코드는 별도 `autobit.live` 경계에 구현되어 있지만, 이번 릴리스에서는 자격증명 접근·서명·인증 요청·실전 원장 생성 전에 **항상 잠금 오류**로 종료됩니다. 환경변수·CLI·UI 해제 스위치는 없습니다. [잠긴 실전 연결 계약과 한계](docs/locked-live-contract.md)를 참고하십시오. 오프라인 테스트 통과는 실전 운용 승인이나 준비 완료를 뜻하지 않습니다.
