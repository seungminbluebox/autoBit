...
전략별 규칙·실험·재현 근거는 [전략 연구 기록](docs/research/README.md)에서 확인합니다.
...
공통 전략 엔진을 사용하는 실전 연결 코드는 별도 `autobit.live` 경계에 구현되어 있지만, 이번 릴리스에서는 자격증명 접근·서명·인증 요청·실전 원장 생성 전에 **항상 잠금 오류**로 종료됩니다. 환경변수·CLI·UI 해제 스위치는 없습니다. [잠긴 실전 운영 안내](docs/runbooks/locked-live.md)와 [연결 계약과 한계](docs/locked-live-contract.md)를 참고하십시오. 오프라인 테스트 통과는 실전 운용 승인이나 준비 완료를 뜻하지 않습니다.

세 모드는 `autobit.core`의 같은 현재 정책을 사용합니다. 공유 정책 변경은 **같은 코드 버전과 일치하는 전략·위험 설정으로 다음 실행**할 때 반영됩니다. 실행 중 hot reload·자동 배포·과거 원장 재작성은 하지 않으며, 실제 체결과 모사 체결의 일치를 보장하지 않습니다. 기존 risk v1은 불변 과거 증거로 검증하고 새로운 판단은 v2로 기록합니다.
...
시작·상태 확인·정상 종료·SQLite backup/restore 검증·장애 자동 복구·증거 확인·모의매매 졸업 기준은 [정규화 모의매매 운영 Runbook](docs/paper-trading-runbook.md)을 따르십시오. OCI의 paper-only systemd 배포·상태·로그·Telegram 자격증명 분리·통제된 service restart 절차는 별도의 [OCI 정규화 모의매매 운영 Runbook](docs/runbooks/oci-paper.md)에 있습니다. 이 문서는 최소 2주와 30회 거래 관찰 기간을 아직 완료했다고 주장하지 않습니다. `PASS`는 paper evaluation 지속만 허용하며, 실거래를 켜거나 매수를 추천하지 않습니다.
...
OCI 서비스는 홈 디렉터리의 legacy `.env`를 직접 읽지 않습니다. 승인된 1회 migration 도구가 `TELEGRAM_TOKEN`과 `TELEGRAM_CHAT_ID`만 `/etc/autobit/paper-notify.env`로 복사하고, 서비스는 그 root 전용 파일만 읽습니다. Upbit 키는 복사하거나 서비스 환경에 넣지 않습니다.

## 전략과 위험 규칙 요약

- 확정된 4시간봉만 사용합니다. 지표는 `EMA(200)`, 현재 봉을 제외한 `Donchian(50/20)`, `ATR(14)`입니다.
- 정상 거래당 위험은 현재 자산의 `2%`, BTC 최대 노출은 `70%`입니다. 고점 대비 `15%` 낙폭 사건은 강제 청산과 `72시간` 대기를 한 번 시작합니다. 정확히 72시간부터 다른 정지가 없고 시스템이 정상이면 낙폭이 남아도 축소 재개합니다. 낙폭 10% 이상 / 5% 이상 / 0% 초과 구간의 회복 위험·상한은 각각 0.25%·15% / 0.5%·30% / 1%·50%이며, 이전 최고자산 회복 시 정상으로 돌아갑니다. 독립 정지·축소 조건은 계속 더 엄격하게 적용합니다.
...
실행 가능한 연구·모의 CLI가 요청하는 업비트 기능은 공개 `KRW-BTC` 캔들뿐입니다. 별도 고정 잠금 live 경계에는 계정·주문 가능 정보·시장가 주문·단건 조회/취소 코드가 있으나 이번 릴리스에서는 인증 요청 이전에 거부됩니다. 개인 WebSocket·입출금 기능과 잠금 해제 스위치는 없습니다. Telegram을 켰을 때만 알림용 `sendMessage` endpoint를 사용합니다. normalized100 원장을 실제 KRW 원장으로 바꿔 표시하지 마십시오. 미확정 live identifier는 재전송하거나 지우지 않고 조정 대상 증거로 유지하는 계약입니다.
