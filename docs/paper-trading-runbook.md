...
이 문서의 PowerShell 명령은 로컬 수동 실행을 위한 것이다. OCI의 paper-only systemd 배포, 원격 상태·로그 조회, 통제된 service restart와 root-only 증거 절차는 [OCI 정규화 모의매매 운영 Runbook](runbooks/oci-paper.md)을 따른다.
...
정상 수집 창은 계속 601봉이며 최장 보유는 1,095봉이다. 신규 진입은 신호 시점의 ATR·리스크 한도를 원장에 고정하고, 실제 시가와 비용을 반영한 체결 한도로 수량을 다시 제한한다. 요청 수량보다 늘리지 않는다. 진입봉의 초기 손절을 먼저 평가하고, 살아남은 포지션의 종가 시점 추적 손절은 다음 봉부터 적용한다.

보유 중 최고가와 초기 위험은 재시작 가능한 관측 이벤트로 보존한다. 이전 형식 원장에 보유기간 증거가 부족하면 최초 BUY·초기 HARD_STOP을 기준으로 필요한 보유 구간만 공개 데이터로 자동 보충한다(최대 1,096개 캔들). 이 구간의 캐시는 시작·종료 시각을 함께 구분하므로 정상 601봉 캐시를 덮어쓰지 않는다. 이전 형식의 미체결 서비스 진입은 원래 신호 창과 당시 리스크 증거를 먼저 복원한다.

공개 과거 데이터가 없거나 검증되지 않으면 값을 추측하지 않고 기존 보호 상태를 보존한 채 자동 재시도한다. 이미 활성화된 손절의 현재 봉 평가는 과거 최고가 복원보다 먼저 수행한다. `paper-status`는 이 복원이나 수집을 실행하지 않는 읽기 전용 조회다. 새 명령, 실제 자금, 거래소 키는 필요하지 않다.
