...
**Interfaces:** The following public types/functions are the stable boundary for Task 3. Existing risk-state fields move into `core.risk_state.RiskState` with compatibility properties/defaults as needed, not a dependency on PaperService. `advance_risk_state(state: RiskState, observation: RiskObservation, *, config: RiskConfig) -> RiskState` receives immutable normalized facts: `now: datetime`, `equity: float`, `closed_trade_pnls: tuple[float, ...]` in full completed-trade order, `volatility_ratio: float`, and `system_healthy: bool`. Trade index validation prevents skipped/double counts; the core owns date baselines, rolling history, cooldown starts and loss/volatility progression. Historical v1 replay remains isolated.

`core.models.PositionContext` holds `entry_price`, `initial_stop`, `current_stop`, `high_water` (floats), `held_bars` (int). `DecisionInput` holds `row: Mapping[str, object]`, `cash: float`, `equity: float`, `position: PositionContext | None`, `has_pending_order: bool`, and `risk: RiskDecision`. `Decision` holds `action: Literal['hold','buy','sell']`, `reason: str | None`, `quantity: float`, `next_stop: float | None`, and `risk: RiskDecision`.
