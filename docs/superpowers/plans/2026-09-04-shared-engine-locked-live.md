...
**Files:** Create `src/autobit/core/__init__.py`, `models.py`, `risk_state.py`, `engine.py`; modify `src/autobit/backtest/engine.py`, `src/autobit/paper/service.py`, `src/autobit/strategy/donchian_trend.py`, `src/autobit/execution/backtest_broker.py`, and `src/autobit/execution/paper_broker.py` only to centralize existing policy and keep compatibility; create `tests/unit/test_shared_decision_engine.py`, `tests/integration/test_shared_mode_policy.py`. Update risk replay imports from Task 1 as needed without changing version semantics. The brokers also contain initial-stop formulas for actual-fill caps and historical execution-context validation: delegate those formulas to one pure strategy helper, preserving persisted entry parameters for old fills instead of recalculating history with new defaults.
